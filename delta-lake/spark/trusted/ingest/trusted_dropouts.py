...
from preprocess.preprocess_dropouts import preprocessing_pipeline
from metadata_manager import add_metadata_entry  # Assuming same metadata system is used
from datetime import datetime

# Spark Session
spark = SparkSession.builder.appName("DropoutsProcessing").getOrCreate()

# Path config
landing_path = '/data/landing/sis_dropouts/'
...
conn.execute("""
    CREATE TABLE IF NOT EXISTS trusted_dropouts (
        dropout_id   INTEGER,
        student_id   VARCHAR,
        dropout_year VARCHAR
    );
""")

# Overwrite existing data
conn.execute("DELETE FROM trusted_dropouts")
conn.register('pdf_df', pdf)
conn.execute("INSERT INTO trusted_dropouts SELECT * FROM pdf_df")

# Verify
count = conn.execute("SELECT COUNT(*) FROM trusted_dropouts").fetchone()[0]
print(f"{count} rows inserted into trusted_dropouts")

conn.close()

# Log metadata
metadata = {
    "source_name":         "trusted_dropouts",
